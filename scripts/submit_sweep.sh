...
for wave in 0 1; do
    ...
    if [[ -n "$previous" ]]; then
        submitted=$(sbatch --parsable --partition=prod10 --gres=gpu:1g.10gb:1 "$@" --array="0-$((count - 1))%10" --dependency="aftercorr:$previous" \
            slurm-prod10.sbatch "$wave")
    else
        submitted=$(sbatch --parsable --partition=prod10 --gres=gpu:1g.10gb:1 "$@" --array="0-$((count - 1))%10" slurm-prod10.sbatch "$wave")
    fi
    previous="${submitted%%;*}"
